...
import { useState, Component as _ReactComponent, lazy, Suspense } from 'react'
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '40px 0 100px' }}>
          <div className="container">
            ...
            {/* Hero */}
            <div style={{ marginBottom: 36, padding: '40px 48px', background: 'linear-gradient(135deg, rgba(245,200,66,0.06) 0%, rgba(0,229,204,0.03) 50%, transparent 100%)', border: '1px solid rgba(245,200,66,0.15)', borderRadius: 20, position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: -60, right: -60, width: 300, height: 300, background: 'radial-gradient(circle, rgba(245,200,66,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
              <div style={{ position: 'absolute', bottom: -40, left: -40, width: 200, height: 200, background: 'radial-gradient(circle, rgba(0,229,204,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16, position: 'relative' }}>
                <div>
                  <div style={{ fontFamily: 'var(--f-mono)', fontSize: 11, fontWeight: 700, color: 'var(--gold)', letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 12 }}>India Wellness Market Intelligence</div>
                  <h1 style={{ fontFamily: 'var(--f-display)', fontSize: 42, fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.02em', marginBottom: 16 }}>
                    <span style={{ background: 'linear-gradient(135deg, #FFE066 0%, #F5C842 50%, #C49A1E 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>DNA Fingerprint</span>
                    <br /><span style={{ color: 'var(--text-1)' }}>Wellness Trends</span>
                    <br /><span style={{ background: 'linear-gradient(135deg, #40FFEE 0%, #00E5CC 50%, #009B8A 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>6 Months Early</span>
                  </h1>
                  ...
                  <div style={{ display: 'flex', gap: 28, marginTop: 24, flexWrap: 'wrap' }}>
                    {[{ val: '1,000+', label: 'Live sources', color: 'var(--gold)' }, { val: '8', label: 'Signal strands', color: 'var(--teal)' }, { val: '95%', label: 'Accuracy rate', color: '#9B6DFF' }, { val: '6mo', label: 'Early warning', color: 'var(--amber)' }].map(function (s) {
                      return (
                        <div key={s.label}>
            ...
            {tab === 'batch' && <BatchScan onAnalyze={goAnalyze} />}
        ...
        <footer style={{ borderTop: '1px solid var(--border-dim)', padding: '18px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8, background: 'var(--bg-raised)' }}>
          <span style={{ fontFamily: 'var(--f-mono)', fontSize: 11, fontWeight: 700, color: 'var(--text-3)' }}>NADI v3.0 — Neural Ayurvedic & Digital Intelligence</span>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div className="pulse-dot" style={{ width: 6, height: 6 }} />
            <span style={{ fontFamily: 'var(--f-mono)', fontSize: 11, color: 'var(--text-3)' }}>Scanning 1,000+ sources · Built for India D2C Founders</span>
          </div>
        </footer>
      </div>
    </>
